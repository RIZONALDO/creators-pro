...
- ~~Self-service signup / billing de SaaS~~ — **implementado na Fase 9.1** (`/signup` + Stripe Checkout/Webhook/Customer Portal, ver [07-roadmap-implementacao.md#fase-91--self-service-signup--billing-stripe](./07-roadmap-implementacao.md#fase-91--self-service-signup--billing-stripe)), depois de pedido direto. O provisionamento manual (`/internal/companies`) continua existindo em paralelo, não foi removido.
- Postgres Row-Level Security (RLS) — recomendado como camada extra de defesa, documentado como item do roadmap de hardening ([07-roadmap-implementacao.md](./07-roadmap-implementacao.md)), mas não bloqueia o MVP porque o isolamento via `tenant_id` + repository pattern já é seguro se aplicado de forma consistente.
  ...
  **Avaliação feita na Fase 9** (não implementado, decisão deliberada — não "ainda não deu tempo"): toda função de repository já recebe `tenantId` explícito e filtra por ele (nunca um `db.select()` sem `WHERE tenant_id`), e a suíte `tenant-isolation.test.ts` cobre explicitamente praticamente toda rota do contrato (20 casos, um por recurso/ação de escrita) — esse é o mecanismo real de defesa hoje, testado, não uma suposição. RLS adicionaria uma camada a mais (o banco rejeita a query mesmo se a aplicação esquecer o filtro), mas tem custo real: toda conexão precisa setar `app.current_tenant_id` via `SET LOCAL` por transação (o Drizzle não faz isso automaticamente), o que muda o `db/client.ts` e exige disciplina equivalente — trocar um lugar de poder esquecer o filtro por outro lugar de poder esquecer o `SET`. Vale revisitar se: (a) a equipe crescer e mais gente passar a tocar repositories sem o mesmo cuidado, ou (b) algum incidente real de leak entre tenants acontecer — nenhum dos dois é o caso agora.
